...
import { getDecartRealtimeModel } from '../libs/decartModel'
...
function waitForRealtimeReady(rt, timeoutMs = 20000) {
  return new Promise((resolve, reject) => {
    ...
    timeoutId = setTimeout(() => {
      cleanup()
      reject(new Error('Timed out waiting for Lucy VTON realtime connection'))
    }, timeoutMs)
...
export function useDecartAI() {
  ...
  const connect = useCallback(async (inputStream, initialPrompt, imageRefUrl = null) => {
    ...
    try {
      ...
      const model = getDecartRealtimeModel()
      ...
      const message = err?.message ?? 'Failed to connect to Lucy VTON'
  ...
  const switchOutfit = useCallback(async (prompt, imageRefUrl = null) => {
    ...
    try {
      ...
      const message = err?.message ?? 'Failed to update Lucy VTON state'
      setLastError(prev => prev || message)
